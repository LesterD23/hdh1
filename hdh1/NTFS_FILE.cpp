...
void NTFS_FILE::getFile(BYTE sectors[]) {
	...
	int tmpId = Hex2Dec(read_offset("2C", 4, sectors));
	_id = tmpId;

    _name = filename(sectors);

	int tmpStatus = Hex2Dec(read_offset("16", 2, sectors));
	_status = tmpStatus;
...
void NTFS_FILE::printFileName() {
	if (_status == 1 || _status == 3)
		cout << "File: " << _name << " (Id: " << _id << ") " << endl;
}

string NTFS_FILE::filename(const BYTE MFT[1024]) {
    string file_name;
    string first_attribute_offset = read_offset_MFT("14", 2, MFT);
    ...
    while (read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset)), 4, MFT) != "ffffffff" && read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset)), 4, MFT) != "FFFFFFFF") {
        if (emergency_break)
            break;
        string attribute = read_offset_MFT(temp_offset, 16, MFT);
        ...
        string attribute_ext_1 = read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset) + 16), size, MFT);
        ...
        if (attribute_type_id == 48) {
            ...
            string attribute_ext_2 = read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset) + Hex2Dec(attribute_data_offset)), size, MFT);
...
void NTFS_FILE::data(LPCWSTR drive, long long id) {
    ...
    // MFT Table
    ReadSector(drive, (MFT_first_sector + id * 2) * bytes_per_sector, tempMFT, 1024);
    string first_attribute_offset = read_offset_MFT("14", 2, tempMFT);
    string temp_offset = first_attribute_offset;
    //long long MFT_Clusters = MFT_clusters(Dec2Hex(Hex2Dec(first_attribute_offset)), drive, sector, tempMFT);
    long long record_ID = Hex2Dec(read_offset_MFT("2C", 2, tempMFT));
    ...
    if (MFT_Name("0", 4, tempMFT) == "FILE") {
        ...
        bool emergency_break = false;
        string temp_offset = first_attribute_offset;
        while (read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset)), 4, tempMFT) != "ffffffff" && read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset)), 4, tempMFT) != "FFFFFFFF") {
            string attribute = read_offset_MFT(temp_offset, 16, tempMFT);
            long long attribute_type_id = Hex2Dec(attribute.substr(24, 8));
            attribute_size = Hex2Dec(attribute.substr(16, 8)) % 0x400;
            bool non_resident_flag = Hex2Dec(attribute.substr(14, 2));
            long long name_length = Hex2Dec(attribute.substr(12, 2));
            ...
            long long size = attribute_size - 16;
            string attribute_ext_1 = read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset) + 16), size, tempMFT);
            long long attribute_data_length = Hex2Dec(attribute_ext_1.substr((size - 4) * 2, 8));
            string attribute_data_offset = attribute_ext_1.substr((size - 6) * 2, 4);
            ...
            if (attribute_type_id == 128) {
                ...
                if (!non_resident_flag) {
                    ...
                    string attribute_ext_2 = read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset) + 32), size, tempMFT);
                    string data_offset = attribute_ext_2.substr((size - 2) * 2, 4);

                    size = attribute_size - Hex2Dec(data_offset);
                    string attribute_ext_3 = read_offset_MFT(Dec2Hex(Hex2Dec(temp_offset) + Hex2Dec(data_offset)), size, tempMFT);
                    ...
                    for (int i = (int)attribute_ext_3.length() - 1; i >= 0; i--) {
                        ...
                        for (long long j = fragment_cluster * sectors_per_cluster; cluster_count_parse > 0 && j < (fragment_cluster + cluster_count) * sectors_per_cluster; j++) {
                            ReadSector(drive, j * bytes_per_sector, tempSector, 512);
